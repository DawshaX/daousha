...
import { Play, Save, Send, ShieldAlert } from "lucide-react";
...
export default function ReviewPublishDesk() {
  ...
  const preflight = trpc.daousha.preflightVettedYouTubeVideo.useMutation({
    onSuccess: result => {
      ...
      setPrivateRequired(false);
      setPublicReady(true);
      toast.success("اجتاز الفيديو الحواجز. يمكن الآن نشره أو جدولة الحزمة المعتمدة مسبقًا من الخلفية.");
    },
  ...
  return (
    <Card className="border-red-500/20 bg-[linear-gradient(145deg,rgba(69,10,10,.28),rgba(9,9,11,.9))]">
      <CardHeader className="border-b border-white/8 pb-4">
        ...
        <CardDescription className="mt-2 leading-6 text-zinc-500">يفحص هذا المكتب الحواجز دون رفع أولًا. إقرار المعاينة المحفوظ هو اعتماد الحزمة قبل الجدولة؛ لا يلزم تأكيد متصفح جديد لكل تنفيذ دوري.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4 p-5">
        {!project || !videoAsset ? <div className="rounded-xl border border-dashed border-white/10 bg-black/20 p-5 text-sm text-zinc-500">أضف مشروعًا غير منشور وملف فيديو معتمدًا لتفعيل مكتب النشر.</div> : <>
          ...
          <div className="grid gap-3 md:grid-cols-2"><div className="rounded-xl border border-white/8 bg-black/20 p-3"><p className="text-[11px] text-zinc-500">إقرار المعاينة</p><p className="mt-1 text-sm text-zinc-100">{previewSaved ? "اعتماد الحزمة محفوظ على الخادم" : "مطلوب من بوابة المراجعة"}</p></div><div className="rounded-xl border border-white/8 bg-black/20 p-3"><p className="text-[11px] text-zinc-500">حالة الفحص</p><p className="mt-1 text-sm text-zinc-100">{publicReady ? "جاهز للنشر أو الجدولة" : privateRequired ? "يتطلب نسخة خاصة" : "لم يُفحص بعد"}</p></div></div>
