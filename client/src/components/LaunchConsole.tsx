...
export default function LaunchConsole() {
  ...
  const projectMutation = trpc.daousha.createProject.useMutation({
    onSuccess: () => { setProjectTitle(""); setProjectBrief(""); refresh(); toast.success("تم إنشاء مشروع جديد داخل غرفة دعوشة."); },
  ...
  return <section className="grid gap-4 xl:grid-cols-3">
    <Card className="border-red-500/18 bg-zinc-950/60"><CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-sm text-white"><FolderKanban className="h-4 w-4 text-red-400" /> مشروع جديد</CardTitle><CardDescription className="text-xs text-zinc-600">ابدأ الفكرة دون إطلاق أي مهمة خارجية.</CardDescription></CardHeader><CardContent><form className="space-y-2" onSubmit={event => { event.preventDefault(); projectMutation.mutate({ title: projectTitle, brief: projectBrief || undefined, targetLanguage: "both" }); }}><input value={projectTitle} onChange={event => setProjectTitle(event.target.value)} required minLength={3} placeholder="عنوان الفيديو أو الحملة" className={inputClass} /><input value={projectBrief} onChange={event => setProjectBrief(event.target.value)} placeholder="موجز اختياري" className={inputClass} /><Button type="submit" disabled={projectMutation.isPending} className="w-full bg-red-600 hover:bg-red-500">{projectMutation.isPending ? "جارٍ الحفظ…" : "إنشاء المشروع"}</Button></form></CardContent></Card>
