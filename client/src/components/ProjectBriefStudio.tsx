  ...
  return <section className="grid gap-5 xl:grid-cols-[1.1fr_.9fr]">
    <Card className="border-red-500/18 bg-zinc-950/60">
      ...
      <CardContent className="space-y-4">
        ...
        <Button disabled={createProject.isPending || title.trim().length < 3} onClick={submit} className="w-full bg-red-600 hover:bg-red-500">{createProject.isPending ? <Loader2 className="ml-2 h-4 w-4 animate-spin" /> : <FolderPlus className="ml-2 h-4 w-4" />}إنشاء المشروع</Button>
    ...
    <Card className="border-white/8 bg-zinc-950/60"><CardHeader><CardTitle className="flex items-center gap-2 text-white"><Clapperboard className="h-5 w-5 text-red-400" /> مشاريع الاستوديو</CardTitle><CardDescription className="mt-2 text-zinc-500">لا تتحول أي بطاقة هنا إلى نشر؛ تسجل فقط تقدم الإنتاج داخل المنصة.</CardDescription></CardHeader><CardContent className="space-y-3">{recentProjects.length ? recentProjects.map(project => <div key={project.id} className="rounded-xl border border-white/8 bg-white/[0.02] p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold text-zinc-200">{project.title}</p><p className="mt-1 line-clamp-2 text-xs leading-5 text-zinc-600">{project.brief || "بانتظار موجز تفصيلي."}</p></div><Badge variant="outline" className="shrink-0 border-white/10 text-[10px] text-zinc-400">{project.status}</Badge></div><div className="mt-3 flex gap-2 text-[10px] text-zinc-500"><span className="inline-flex items-center gap-1"><FileText className="h-3 w-3" />{project.targetLanguage.toUpperCase()}</span><span>·</span><span>{project.contentFormat === "short" ? "Short" : "Long"}</span></div></div>) : <div className="rounded-xl border border-dashed border-white/10 bg-black/20 p-6 text-center text-sm text-zinc-600">أنشئ أول مشروع من الموجز لتظهر حزمته هنا.</div>}</CardContent></Card>
  </section>;
}
