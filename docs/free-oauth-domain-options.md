...
* فحص DNS وHTTPS في `2026-08-14` أكد أن `xdawnova.int.eu.org` غير قابل للحل العام بعد؛ لذلك لا يمكن تجربة الموقع أو Facebook OAuth عبره الآن. تشير إرشادات EU.org الرسمية إلى أن التحقق يحتاج تدخلًا بشريًا وقد يستغرق بضعة أيام قبل إرسال رسالة التحقق. [4]
* تنبيه المتابعة مفعّل الآن كل ست ساعات عبر مهمة Heartbeat `xdaw-euorg-domain-monitor`. يفحص التفويض العام لخادمي Cloudflare ويرسل Telegram عند بدء المتابعة أو عند تحول الحالة إلى `delegated` فقط، لتفادي الرسائل المتكررة أثناء انتظار المراجعة.
