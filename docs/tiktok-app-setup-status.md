...
بعد تفويض Sandbox المؤقت، اكتمل في `2026-08-14` رفع **Reel 02 English — Verify Before You Share**، وهو أصل مملوك معتمد الحقوق وواضح السلامة، إلى **Inbox Draft** في TikTok Sandbox. أعادت واجهة TikTok معرّف التتبع `v_inbox_file~v2.7673821700143810576` وأكدت المنصة أنه لم يُنشر أي محتوى. استخدم الاختبار نقل `FILE_UPLOAD` إلى المسودة فقط، وليس Direct Post أو النشر العام.

حُذف فيديو المراجعة المبدئي من مسودة Production واستُبدل بالملف `xdawnova-tiktok-review-demo-sandbox-live.mp4`، ثم ظهرت رسالة TikTok **Saved** في `2026-08-14`. بقي زر `Submit for review` غير مستخدم. يعرض الفيديو الجديد سجل OAuth ورفع Sandbox الفعلي ومعرّف المسودة داخل واجهة XDAW NOVA، لكنه لا يلتقط نافذة موافقة TikTok نفسها أو نقرة المستخدم داخلها؛ لذلك لا يُرسل التطبيق للمراجعة بعد. يلزم تسجيل شاشة تفاعلي كامل لإعادة OAuth المقيد وإرفاقه إذا أراد المالك أن يطابق دليل الفيديو حرفيًا مع متطلب TikTok لإظهار تفاعلات المستخدم.

## جاهزية تفويض الإنتاج بعد الموافقة

حُدّث مسار TikTok الإنتاجي بحيث يحفظ رمز الوصول ورمز التجديد مشفرين خادميًا، ويسجل زمن انتهاء رمز الوصول في `credentialExpiresAt`. عند انتهاء الرمز أو اقترابه من الانتهاء، يُجدد الخادم الرمز من رمز التجديد عبر نقطة TikTok OAuth ثم يستبدل الحزمة المشفرة. لا تقرأ هذه الآلية ملف تعريف المتصفح ولا تستخدم رمز Sandbox؛ ولا تعمل إلا بعد موافقة TikTok الرسمية ثم تفويض OAuth إنتاجي منفصل. اجتاز فحص TypeScript واختبار انتهاء الرمز الوحدوي.
